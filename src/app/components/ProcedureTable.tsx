...
function newRow(): ProcedureRow {
  return { id: crypto.randomUUID(), icd10: '', start: '', location: '', service: '', physician: '' };
}
...
export function ProcedureTable() {
  ...
  const update = (id: string, field: keyof ProcedureRow, value: string) =>
        ...
        <table className="w-full text-xs border-collapse">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              ...
              <th className="px-2 py-2 text-left font-semibold text-gray-600 whitespace-nowrap">Start</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {rows.map((row) => (
              <tr key={row.id} className="hover:bg-gray-50/60 transition-colors">
                ...
                {/* ICD-10 — always visible */}
                <td className="px-2 py-1.5">
                  <input
                    type="text"
                    maxLength={7}
                    placeholder="Select..."
                    className={`${inp} w-[72px] font-mono uppercase`}
                    value={row.icd10}
                    onChange={e => update(row.id, 'icd10', e.target.value.toUpperCase())}
                  />
                </td>

                {/* Dependent columns — only when ICD-10 is filled */}
                {row.icd10.trim() ? (
                  <>
                    <td className="px-2 py-1.5">
                      <input type="datetime-local" className={`${inp} w-[170px]`}
                        value={row.start} onChange={e => update(row.id, 'start', e.target.value)} />
                    </td>
                    <td className="px-2 py-1.5">
                      <select className={`${sel} w-[130px]`}
                        value={row.location} onChange={e => update(row.id, 'location', e.target.value)}>
                        <option value="">Select...</option>
                        {LOCATIONS.map(l => <option key={l}>{l}</option>)}
                      </select>
                    </td>
                    <td className="px-2 py-1.5">
                      <select className={`${sel} w-[150px]`}
                        value={row.service} onChange={e => update(row.id, 'service', e.target.value)}>
                        <option value="">Select...</option>
                        {SERVICES.map(s => <option key={s}>{s}</option>)}
                      </select>
                    </td>
                    <td className="px-2 py-1.5">
                      <select className={`${sel} w-[140px]`}
                        value={row.physician} onChange={e => update(row.id, 'physician', e.target.value)}>
                        <option value="">Select...</option>
                        {PHYSICIANS.map(p => <option key={p}>{p}</option>)}
                      </select>
                    </td>
                  </>
                ) : (
