...
export function AccordionChecklist({ title, items, values, onChange, showCount = true }: AccordionChecklistProps) {
    ...
    <div className="border border-gray-200 rounded-md overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {showCount && checkedCount > 0 && (
            <span className="text-xs font-semibold px-1.5 py-0.5 rounded-full bg-primary text-white leading-none">
              {checkedCount}
            </span>
          )}
        </div>
